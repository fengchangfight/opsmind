...
python -c "from opmind.retrieval.vector_store import VectorStore; c = VectorStore().count(); exit(0 if c > 0 else 1)" 2>nul
if %errorlevel% neq 0 (
    echo [INFO] No indexed data found. Running ingestion...
    echo [INFO] This may take 2-3 minutes on first run (downloading embedding model + indexing)
...
start "OpsMind Backend" cmd /k "uvicorn opmind.api.main:app --host 0.0.0.0 --port 8000 --reload"
