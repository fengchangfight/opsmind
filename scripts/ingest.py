"""LlamaIndex SentenceSplitter + dual embedding (dense + BM25 sparse) → Milvus."""
import asyncio
import hashlib
import json
import os
...
async def main():
    ...
    vector_store.clear()
    print("[Ingest] Created collection (dense + sparse fields)")

    cache = _load_cache()
    print(f"[Ingest] Cache: {len(cache)} previously indexed docs")

    # Phase 1: Load + filter
    sp = SentenceSplitter(chunk_size=settings.chunk_size, chunk_overlap=settings.chunk_overlap)
    ...
    # Phase 2: Chunk
    nodes = sp(li_docs) if li_docs else []
    print(f"[Ingest] {len(li_docs)} docs → {len(nodes)} chunks")
    ...
    if nodes:
        ...
        for i in range(0, len(nodes), BATCH):
            ...
            chunks = [Chunk(chunk_id=n.node_id, doc_id=n.metadata.get("doc_id", ""), content=n.text,
                            metadata={"doc_id": n.metadata.get("doc_id", ""),
            ...
            if (i + BATCH) % 128 == 0:
                print(f"[Ingest] {min(i + BATCH, len(nodes))}/{len(nodes)} chunks embedded")

    # Phase 4: Cleanup
    removed = set(cache.keys()) - set(new_cache.keys())
    for rid in removed:
        vector_store.delete_by_doc_id(rid)
    if removed:
        print(f"[Ingest] Removed {len(removed)} stale docs")
